...
class Bfpi_Importer {
    ...
    public function start_import($import_data) {
        ...
        try {
            ...
            // Format specifiers for import_record columns
            $import_record_formats = array('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%s', '%s', '%d', '%d', '%s');
            ...
            if ($is_update) {
                ...
                if ($result === false) {
                    ...
                    throw new Exception(esc_html__('Failed to update import record: ', 'bootflow-product-xml-csv-importer') . esc_html($error));
                }
                ...
                if ($result === false) {
                    $error = $wpdb->last_error;
                    throw new Exception(esc_html__('Failed to create import record: ', 'bootflow-product-xml-csv-importer') . esc_html($error));
                }
    ...
    private function import_single_product($product_data) {
        try {
            ...
            if (!isset($this->_cached_import_settings)) {
                $this->_cached_import_settings = $wpdb->get_row($wpdb->prepare("SELECT update_existing, skip_unchanged FROM {$wpdb->prefix}bfpi_imports WHERE id = %d", $this->import_id), ARRAY_A); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Custom table query
            }
            ...
            if ($existing_product_id && $update_existing) {
                ...
                if ($skip_unchanged && !$this->has_product_data_changed($existing_product_id, $processed_data)) {
                    ...
                    // handle_missing_products() uses _bfpi_import_date to identify products in the feed.
                    // Without this, skipped products get incorrectly treated as "missing" and moved to draft.
                    update_post_meta($existing_product_id, '_bfpi_import_id', $this->import_id);
                    update_post_meta($existing_product_id, '_bfpi_import_date', current_time('mysql'));
            ...
            if ($final_product && $final_product->is_type('variable')) {
                ...
                if ($variation_count === 0) {
                    ...
                    if (!empty($children)) {
                        ...
                        // Build safe query: each child ID gets its own %d placeholder
                        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
                        $instock_count = (int) $wpdb->get_var(
                            // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Query is safely prepared below
                            $wpdb->prepare(
                                "SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE post_id IN (" . implode(',', array_fill(0, count($child_ids), '%d')) . ") AND meta_key = '_stock_status' AND meta_value = 'instock'",
                                $child_ids
                            )
                        );
    ...
    private function import_csv_variable_product($grouped_data) {
        ...
        // Get update settings
        global $wpdb;
        $import_record = $wpdb->get_row($wpdb->prepare("SELECT update_existing FROM {$wpdb->prefix}bfpi_imports WHERE id = %d", $this->import_id), ARRAY_A); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, PluginCheck.Security.DirectDB.UnescapedDBParameter -- Custom table query
        $update_existing = ($import_record && $import_record['update_existing'] === '1') ? true : false;
    ...
    private function map_product_fields($product_data) {
        ...
        if (empty($field_mappings) || !is_array($field_mappings)) {
            ...
            foreach ($field_mappings as $field_key => &$mapping_config) {
        ...
        unset($mapping_config); // CRITICAL: Break reference from foreach to prevent variable leak
        ...
        foreach ($field_mappings as $field_key => $mapping_config) {
            ...
            if (!is_array($mapping_config)) {
                // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
                if (defined('WP_DEBUG') && WP_DEBUG) { error_log('Bootflow Import - Skipping invalid mapping config for ' . $field_key); }
                continue;
            }
            ...
            if (isset($mapping_config['boolean_mode'])) {
                ...
                if ($boolean_mode === 'yes') {
                    $mapped_data[$field_key] = 'yes';
                    continue;
                } elseif ($boolean_mode === 'no') {
                    $mapped_data[$field_key] = 'no';
                    continue;
                }
            ...
            if (isset($mapping_config['select_mode'])) {
                ...
                if ($select_mode === 'fixed' && isset($mapping_config['fixed_value'])) {
                    $mapped_data[$field_key] = $mapping_config['fixed_value'];
                    continue;
                }
                // If mode is 'map', continue to extract from source
            }
            ...
            if ($field_key === 'sku' && isset($mapping_config['sku_mode']) && $mapping_config['sku_mode'] === 'generate') {
                ...
                $mapped_data[$field_key] = $generated_sku;
                // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
...
// Debug: log what we're about to process
        ...
        // If quantity is mapped with a value, ensure we will manage stock
        if (isset($processed_data['stock_quantity']) && $processed_data['stock_quantity'] !== '') {
    ...
    private function create_product($product_data, $mapped_data = array()) {
        ...
        $product_name = !empty($product_data['name']) ? $product_data['name'] : (!empty($product_data['sku']) ? $product_data['sku'] : __('Product', 'bootflow-product-xml-csv-importer'));
        ...
        if (isset($product_data['menu_order'])) {
            $product->set_menu_order((int) $product_data['menu_order']);
        }
        ...
        update_post_meta($product_id, '_bfpi_import_id', $this->import_id);
        update_post_meta($product_id, '_bfpi_import_date', current_time('mysql'));
        update_post_meta($product_id, '_bfpi_import_source', 'create');
...
// Check if Pricing Engine is active (needs full pipeline for price calculations)
        ...
        if ($skip_unchanged) {
            ...
            if (!$changed) {
                // CRITICAL: Still update import tracking even for unchanged products!
                // handle_missing_products() uses _bfpi_import_date to identify products that were in the feed.
    ...
    private function update_product($product_id, $product_data, $mapped_data = array()) {
        ...
        if ($this->should_update_field('name', $product_data['name'] ?? null, false)) {
            // Fallback to default name if empty (use SKU or "Product" as fallback)
            $product_name = !empty($product_data['name']) ? $product_data['name'] : (!empty($product_data['sku']) ? $product_data['sku'] : __('Product', 'bootflow-product-xml-csv-importer'));
            $product->set_name($product_name);
        }
        ...
        if (isset($product_data['stock_quantity']) && $product_data['stock_quantity'] !== '' && $this->should_update_field('stock_quantity', $product_data['stock_quantity'], false)) {
            $product->set_manage_stock(true);
        ...
        if ($this->should_update_field('purchase_note', $product_data['purchase_note'] ?? null, false)) {
            $product->set_purchase_note($product_data['purchase_note'] ?? '');
        }

        // Update menu order
        if ($this->should_update_field('menu_order', $product_data['menu_order'] ?? null, false)) {
            $product->set_menu_order((int) ($product_data['menu_order'] ?? 0));
        }

        // Handle External product specific fields
        if ($product instanceof WC_Product_External) {
            if ($this->should_update_field('external_url', $product_data['external_url'] ?? null, false)) {
                $product->set_product_url($product_data['external_url'] ?? '');
            }
            if ($this->should_update_field('button_text', $product_data['button_text'] ?? null, false)) {
                $product->set_button_text($product_data['button_text'] ?? '');
            }
        }
        ...
        // Handle SEO meta fields
        if ($this->should_update_field('meta_title', $product_data['meta_title'] ?? null, false) || $this->should_update_field('meta_description', $product_data['meta_description'] ?? null, false) || $this->should_update_field('meta_keywords', $product_data['meta_keywords'] ?? null, false)) {
            $this->set_product_seo_meta($product_id, $product_data);
        }
        ...
        update_post_meta($product_id, '_bfpi_import_id', $this->import_id);
        update_post_meta($product_id, '_bfpi_import_date', current_time('mysql'));
        update_post_meta($product_id, '_bfpi_import_source', 'update');
    ...
    private function set_product_seo_meta($product_id, $product_data) {
        ...
        // Generic fallback - always save to standard meta keys
        if (!empty($product_data['meta_title'])) {
            update_post_meta($product_id, '_meta_title', $product_data['meta_title']);
        }
        if (!empty($product_data['meta_description'])) {
            update_post_meta($product_id, '_meta_description', $product_data['meta_description']);
        }
        if (!empty($product_data['meta_keywords'])) {
            update_post_meta($product_id, '_meta_keywords', $product_data['meta_keywords']);
        }
    }
    ...
    private function parse_image_placeholders($template, $product_data) {
        ...
        foreach ($entries as $entry) {
            ...
            if (preg_match('/^([a-zA-Z0-9_]+)\*$/', $entry, $matches) || preg_match('/^\{([a-zA-Z0-9_]+)\*\}$/', $entry, $matches)) {
                ...
                } elseif (isset($product_data[$field_base]) && !empty($product_data[$field_base])) {
                    // FALLBACK: Try base field with comma-separated values
                    $base_val = $this->extract_text_value($product_data[$field_base]);
                    if (!empty($base_val) && strpos($base_val, ',') !== false) {
                        $comma_parts = array_filter(array_map('trim', explode(',', $base_val)));
                        $result_urls = array_merge($result_urls, $comma_parts);
                        $this->log('debug', "WILDCARD: Split base field '$field_base' by comma, got " . count($comma_parts) . " URLs");
                    } elseif (!empty($base_val)) {
                        $result_urls[] = $base_val;
                    }
                } else {
            ...
            if (!empty($variations_data)) {
                ...
                if ($need_auto_detect) {
                    ...
                    if (!empty($attrs_data) && is_array($attrs_data)) {
                        ...
                        // ════════════════════════════════════════════════════════
                        // FORMAT C: Attributes are DIRECT children of <variation>
                        // e.g., <variation><color>Red</color><size>S</size><sku>...</sku></variation>
                        // (not nested under <attributes>)
                        // ════════════════════════════════════════════════════════
                        $auto_attributes = array();
                        
                        // Strategy 1: Use parent product's <attributes> block to identify which keys are attributes
                        $parent_attrs = isset($product_data['attributes']) && is_array($product_data['attributes']) ? $product_data['attributes'] : null;
                        if (!empty($parent_attrs)) {
                            foreach (array_keys($parent_attrs) as $attr_key) {
                                if (is_numeric($attr_key) || $attr_key === '@attributes') {
                                    continue;
                                }
                                // Collect values for this attribute from all variations
                                $all_values = array();
                                foreach ($variations_data as $var) {
                                    if (isset($var[$attr_key]) && is_string($var[$attr_key]) && $var[$attr_key] !== '') {
                                        $all_values[] = trim($var[$attr_key]);
                                    }
                                }
                                $all_values = array_unique($all_values);
                                if (!empty($all_values)) {
                                    $auto_attributes[] = array(
                                        'name' => ucfirst($attr_key),
                                        'xml_attribute_key' => $attr_key,
                                        'values_source' => '',
                                        'visible' => 1,
                                        'used_for_variations' => 1,
                        ...
                        // Strategy 2: Scan variation data directly, excluding known WooCommerce fields
                        if (empty($auto_attributes)) {
                            ...
                            foreach ($first_var as $key => $val) {
                                if (is_numeric($key) || !is_string($val) || in_array(strtolower($key), $skip_fields)) {
                                    continue;
                                }
                                $all_values = array();
                                foreach ($variations_data as $var) {
                                    if (isset($var[$key]) && is_string($var[$key]) && $var[$key] !== '') {
                                        $all_values[] = trim($var[$key]);
                                    }
                                }
                                $all_values = array_unique($all_values);
                                if (!empty($all_values)) {
    ...
    private function get_variation_field($var_data, $map_config, $field_name, $default_paths = array()) {
        ...
        if (!empty($mapped_source)) {
            ...
            // E.g., "variations.variation[3].sku" → "sku"
            // E.g., "variations.variation.sku" → "sku"
            $container_xpath = $map_config['container_xpath'] ?? 'variations.variation';
            
            // Use regex to strip container_xpath with any array index: container[N].
            $escaped_xpath = preg_quote($container_xpath, '/');
            if (preg_match('/^' . $escaped_xpath . '(?:\[\d+\])?\\.(.+)$/', $clean_source, $prefix_match)) {
                $clean_source = $prefix_match[1];
            } else {
                // Also try common defaults
                if (preg_match('/^variations\.variation(?:\[\d+\])?\\.(.+)$/', $clean_source, $prefix_match)) {
                    $clean_source = $prefix_match[1];
                }
            }
        ...
        $grouped_products = $wpdb->get_results($wpdb->prepare(
            "SELECT p.ID, pm.meta_value as pending_skus
             ...
             WHERE pm.meta_key = '_pending_grouped_skus'
               AND pm_import.meta_key = '_bfpi_import_id'
    ...
    private function handle_missing_products() {
        ...
        // 1. Have _bfpi_import_id = current import ID (were last updated by this import)
        // 2. Have _bfpi_import_date OLDER than import start time (not updated in THIS run)
        ...
        $missing_products = $wpdb->get_results($wpdb->prepare(
            "SELECT p.ID, p.post_title, pm_sku.meta_value as sku, pm_date.meta_value as import_date
             FROM {$wpdb->posts} p
             INNER JOIN {$wpdb->postmeta} pm_import ON p.ID = pm_import.post_id AND pm_import.meta_key = '_bfpi_import_id'
             LEFT JOIN {$wpdb->postmeta} pm_sku ON p.ID = pm_sku.post_id AND pm_sku.meta_key = '_sku'
             LEFT JOIN {$wpdb->postmeta} pm_date ON p.ID = pm_date.post_id AND pm_date.meta_key = '_bfpi_import_date'
        ...
        foreach ($missing_products as $product) {
            try {
                ...
                if ($delete_variations && $product->is_type('variable') && $action !== 'delete') {
    ...
    private function check_shipping_rule_conditions($rule, $product_data) {
        ...
        foreach ($conditions as $condition) {
            ...
            switch ($type) {
                ...
                case 'price_range':
                    // Use the ORIGINAL source price (before Pricing Engine markup) for shipping class rules.
                    // $product_data contains the post-markup price, but shipping rules should evaluate
                    // against the supplier/source price from the XML/CSV feed.
                    $price = 0;
                    $pricing_config = $this->config['field_mapping']['pricing_engine'] ?? null;
                    if (!empty($pricing_config['enabled']) && !empty($pricing_config['base_price']) && !empty($this->_current_raw_product_data)) {
                        // Extract original price using the same base_price template as Pricing Engine
                        $base_template = $pricing_config['base_price'];
                        $original_price_str = $this->parse_template_string($base_template, $this->_current_raw_product_data);
                        if ($original_price_str !== '') {
                            $price = floatval(str_replace(',', '.', preg_replace('/[^0-9.,]/', '', $original_price_str)));
                        }
                    }
                    // Fallback to processed price if no pricing engine or no raw data
                    if ($price <= 0) {

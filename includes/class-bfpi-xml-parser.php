...
class Bfpi_XML_Parser {
    ...
    public function parse_structure($file_path, $product_wrapper, $page = 1, $per_page = 5, $scan_for_fields = 0) {
        try {
            ...
            if (function_exists('set_time_limit')) { @set_time_limit(300); } // phpcs:ignore Squiz.PHP.DiscouragedFunctions.Discouraged, WordPress.PHP.NoSilencedErrors.Discouraged -- Required for long-running imports
    ...
    public function count_products_and_extract_structure($file_path, $product_wrapper) {
        try {
            // Set time and memory limits for large files
            if (function_exists('set_time_limit')) { @set_time_limit(600); } // phpcs:ignore Squiz.PHP.DiscouragedFunctions.Discouraged, WordPress.PHP.NoSilencedErrors.Discouraged -- Required for long-running imports
            wp_raise_memory_limit('admin');

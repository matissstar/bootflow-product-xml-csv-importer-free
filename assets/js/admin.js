...
window.addDisplayAttributeRowGlobal = function(targetContainer) {
    ...
    void 0 && console.log('★★★ GLOBAL addDisplayAttributeRowGlobal() CALLED, target:', targetContainer);
...
(function($) {
    ...
    function initializeStep() {
        if ($('.bfpi-step-1').length) {
            initializeStep1();
        } else if ($('.bfpi-step-2').length) {
            initializeStep2();
        } else if ($('.bfpi-step-3').length) {
            initializeStep3();
        }
    }
    ...
    function initializeStep1() {
        void 0 && console.log('Initializing Step 1: File Upload');
        ...
        // Form submission
        $('#bfpi-upload-form').on('submit', handleUploadSubmission);
    }
    ...
    function initializeStep2() {
        void 0 && console.log('Initializing Step 2: Field Mapping');
        
        // Detect and set file type
        if (typeof bfpiImportData !== 'undefined' && bfpiImportData.file_type) {
            $('#detected_file_type').val(bfpiImportData.file_type);
            void 0 && console.log('★★★ Set detected_file_type to:', bfpiImportData.file_type);
        } else if (typeof bfpiImportData !== 'undefined' && bfpiImportData.file_path) {
            // Fallback: detect from file extension
            var ext = bfpiImportData.file_path.split('.').pop().toLowerCase();
            if (ext === 'csv') {
                $('#detected_file_type').val('csv');
            } else {
                $('#detected_file_type').val('xml');
            }
            void 0 && console.log('★★★ Detected file type from extension:', ext);
        }
        
        // Load saved recipes into dropdown
        loadRecipesList();

        // Load file structure if bfpiImportData.file_path is available (new import or edit mode)
        if (typeof bfpiImportData !== 'undefined' && bfpiImportData.file_path) {
            loadFileStructure();
            // Note: loadSavedMappings is now called inside loadFileStructure success callback
            // after dropdowns are populated
        } else {
            void 0 && console.log('Skipping loadFileStructure - no file data available');
            // No file structure to load, but still load saved mappings if available
            if (typeof bfpiImportData !== 'undefined' && bfpiImportData.saved_mappings) {
                void 0 && console.log('★★★ LOADING SAVED MAPPINGS (no file structure) ★★★');
                loadSavedMappings(bfpiImportData.saved_mappings);
            }
            // Initialize engines immediately since there will be no file structure
            if (!window._bfpiEnginesInitialized) {
                window._bfpiEnginesInitialized = true;
                initPricingEngine();
                initShippingClassEngine();
                if (typeof bfpiImportData !== 'undefined' && bfpiImportData.saved_mappings) {
                    if (bfpiImportData.saved_mappings.pricing_engine) {
                        window.restorePricingEngineConfig(bfpiImportData.saved_mappings.pricing_engine);
                    }
                    if (bfpiImportData.saved_mappings.shipping_class_engine) {
                        window.restoreShippingClassEngineConfig(bfpiImportData.saved_mappings.shipping_class_engine);
                    }
                }
            }
        }
        ...
        if (typeof bfpiImportData !== 'undefined' && bfpiImportData.saved_custom_fields && bfpiImportData.saved_custom_fields.length > 0) {
            setTimeout(function() {
                loadSavedCustomFields(bfpiImportData.saved_custom_fields);
        ...
        $(document).on('change', '.processing-mode-select', function() {
            ...
            if (mode === 'direct') {
                ...
                void 0 && console.log('★ Showing config panel:', panelClass, 'Found:', configDiv.find(panelClass).length);
            }
        });
        ...
        // Form submission
        $('#bfpi-mapping-form').on('submit', handleMappingSubmission);
    ...
    function initializeAiAutoMapping() {
        ...
        $('#btn-ai-auto-map').on('click', function() {
            ...
            if (window.currentSampleData && window.currentSampleData.length > 0) {
                ...
                void 0 && console.log('★★★ AI Auto-Mapping - Sending ' + maxProducts + ' products for analysis');
            }
            ...
            void 0 && console.log('★★★ AI Auto-Mapping - Sending request', {
            ...
            // Send AJAX request
            $.ajax({
                url: bfpiImportData.ajax_url,
                type: 'POST',
                data: {
                    action: 'bfpi_auto_mapping',
                    nonce: bfpiImportData.nonce,
                ...
                success: function(response) {
                    void 0 && console.log('★★★ AI Auto-Mapping Response:', response);
                    $progress.hide();
                    ...
                    if (response.success) {
                        ...
                        if (productStructure && productStructure.has_variations) {
                            void 0 && console.log('★★★ Variable product detected - enabling Variable mode FIRST');
                            applyProductStructure(productStructure);
                            
                            // Wait for DOM to update, then apply ALL mappings including variation fields
                            setTimeout(function() {
                                void 0 && console.log('★★★ Now applying mappings after Variable mode is enabled');
                            ...
                            if (hasAttributeMappings) {
                                void 0 && console.log('★★★ Attributes detected in simple product - enabling "With Attributes" mode');
    ...
    function applyDisplayAttributes(mappings) {
        ...
        $.each(mappings, function(sourceField, targetField) {
            if (typeof targetField === 'string' && targetField.indexOf('attribute:') === 0) {
                ...
                if (sourceField.indexOf('@attributes') !== -1) {
                    void 0 && console.log('★★★ Skipping XML metadata field:', sourceField);
                    return true; // continue
                }
                // Skip #text fields from attribute containers (handled by auto-detect)
                if (/attributes\.attribute.*#text/.test(sourceField)) {
                    void 0 && console.log('★★★ Skipping attribute #text field:', sourceField);
                    return true; // continue
                }
        ...
        void 0 && console.log('★★★ applyDisplayAttributes() - found attributes:', attrs);
        ...
        if ($attrRadio.length > 0) {
            ...
            void 0 && console.log('★★★ Set product_mode to: attributes');
        }
        ...
        setTimeout(function() {
            if (isKeyValue) {
                ...
                void 0 && console.log('★★★ Detected Key-Value attribute pattern');
                ...
                setTimeout(function() {
                    ...
                    setTimeout(function() {
                        ...
                        if ($row.length > 0) {
                            ...
                            void 0 && console.log('★★★ Set Key-Value pair:', nameFieldPath, '→', valueFieldPath);
                        }
                    }, 200);
                }, 200);
            } else {
                // Standard mode — one row per attribute
                void 0 && console.log('★★★ Using Standard attribute mode');
                var attrIndex = 0;
                $.each(attrs, function(attrName, sourceField) {
                    // Add attribute row
                    ...
                    (function(name, source, idx) {
                        setTimeout(function() {
                            ...
                            if ($row.length > 0) {
                                ...
                                void 0 && console.log('★★★ Added display attribute:', displayName, '→ {' + source + '}');
                            }
                        }, 150 * (idx + 1));
    ...
    function applyProductStructure(productStructure) {
        void 0 && console.log('★★★ applyProductStructure() called with:', productStructure);
        ...
        if ($variableRadio.length > 0) {
            ...
            void 0 && console.log('★★★ Set product_mode to: variable');
        }
        ...
        if (productStructure.variation_path) {
            ...
            if ($variationPath.length > 0) {
                ...
                void 0 && console.log('★★★ Set variation path to:', productStructure.variation_path);
            }
            ...
            if ($variationPathTextarea.length > 0) {
                ...
                void 0 && console.log('★★★ Set variation path textarea to:', '{' + productStructure.variation_path + '}');
            }
        }
        
        // Store detected attributes globally for import processor
        if (productStructure.detected_attributes && productStructure.detected_attributes.length > 0) {
            void 0 && console.log('★★★ Detected attributes:', productStructure.detected_attributes);
                    ...
                    setTimeout(function() {
                        ...
                        if ($row.length > 0) {
                            ...
                            if ($nameInput.length > 0) {
                                ...
                                void 0 && console.log('★★★ Set var attribute name ' + index + ' to:', displayName);
                            }
                            ...
                            if ($sourceTextarea.length > 0 && productStructure.variation_path) {
                                ...
                                void 0 && console.log('★★★ Set var attribute source ' + index + ' to:', '{' + sourcePath + '}');
                            }
                        }
                    }, 150 * (index + 1));
                });
    ...
    function applyAttributeMappings(mappings, variationPath) {
        void 0 && console.log('★★★ applyAttributeMappings() called');
        ...
        void 0 && console.log('★★★ Found attribute mappings:', attrMappings);
                ...
                $('#variation-attributes-list .attr-row').each(function() {
                    ...
                    if (currentName === attrName.toLowerCase()) {
                        ...
                        if ($sourceTextarea.length > 0) {
                            ...
                            void 0 && console.log('★★★ Updated attribute source:', attrName, '→', templateValue);
                        }
                    }
                });
            });
    ...
    function applyAiMappings(mappings, confidence) {
        void 0 && console.log('★★★ applyAiMappings() called with:', mappings);
        ...
        $.each(mappings, function(sourceField, targetWcField) {
            void 0 && console.log('★★★ Trying to map:', sourceField, '→', targetWcField);
            ...
            if ($row.length > 0) {
                ...
                if ($textarea.length > 0) {
                    ...
                    mapped = true;
                    void 0 && console.log('★★★ Set textarea value:', templateValue);
                }
                ...
                if (!mapped) {
                    ...
                    if ($sourceSelect.length > 0) {
                        ...
                        if (optionExists) {
                            ...
                            mapped = true;
                            void 0 && console.log('★★★ Set dropdown value:', sourceField);
                        } else {
                            void 0 && console.log('★★★ Source field option not found in dropdown:', sourceField);
                        }
                    }
                }
                ...
                if (mapped) {
                    ...
                    appliedCount++;
                    void 0 && console.log('★★★ Successfully mapped:', sourceField, '→', targetWcField);
                }
            } else {
                ...
                if (varFieldName) {
                    ...
                    if ($varTextarea.length > 0) {
                        ...
                        appliedCount++;
                        void 0 && console.log('★★★ Successfully mapped variation field:', sourceField, '→ var_field[' + varFieldName + ']');
                    } else {
                        void 0 && console.log('★★★ Variation field textarea not found: var_field[' + varFieldName + ']');
                    }
                    ...
                    if ($varTextarea.length > 0) {
                        ...
                        appliedCount++;
                        void 0 && console.log('★★★ Auto-detected variation field:', sourceField, '→ var_field[' + guessedField + ']');
                    } else {
                        void 0 && console.log('★★★ Could not find var_field textarea for:', guessedField, 'from source:', sourceField);
                    }
                } else {
                    void 0 && console.log('★★★ WC field row not found:', targetWcField);
                }
            }
        });
        ...
        if (!skuMapped) {
            void 0 && console.log('★★★ SKU not mapped by AI - enabling Auto-generate mode');
        ...
        void 0 && console.log('★★★ Applied', appliedCount, 'mappings');
    ...
    function performSmartMapping(sourceFields) {
        ...
        $.each(sourceFields, function(i, sourceField) {
            ...
            $.each(smartMappingPatterns, function(targetField, patterns) {
                if (matchedSources[sourceField] || matchedTargets[targetField]) return;
                ...
                for (var j = 0; j < patterns.length; j++) {
                    ...
                    if (normalizedSource === normalizedPattern) {
                        ...
                        matchedTargets[targetField] = true;
                        void 0 && console.log('★★★ Smart match (exact):', sourceField, '→', targetField);
                        return false; // break $.each for this targetField
                    }
                }
            });
        });
            ...
            $.each(smartMappingPatterns, function(targetField, patterns) {
                ...
                for (var j = 0; j < patterns.length; j++) {
                    ...
                    if (normalizedPattern.length >= 3 && normalizedSource.indexOf(normalizedPattern) !== -1) {
                        ...
                        matchedSources[sourceField] = true;
                        matchedTargets[targetField] = true;
                        void 0 && console.log('★★★ Smart match (contains):', sourceField, '→', targetField);
                        return false;
                    }
                    ...
                    if (normalizedSource.length >= 3 && normalizedPattern.indexOf(normalizedSource) !== -1) {
                        ...
                        matchedSources[sourceField] = true;
                        matchedTargets[targetField] = true;
                        void 0 && console.log('★★★ Smart match (reverse-contains):', sourceField, '→', targetField);
                        return false;
                    }
                }
            });
        ...
        void 0 && console.log('★★★ Smart mapping result:', Object.keys(mappings).length, 'fields matched');
        return mappings;
    }
    ...
    function applySmartMappings(mappings) {
        void 0 && console.log('★★★ applySmartMappings() called with:', mappings);
        ...
        // If SKU was not mapped, enable Auto-generate mode
        if (!skuMapped) {
            void 0 && console.log('★★★ SKU not mapped - enabling Auto-generate mode');
            enableSkuAutoGenerate();
        }
        ...
        void 0 && console.log('★★★ Applied', appliedCount, 'smart mappings');
        updateMappedCounts();
    }
    ...
    function smartFallbackForUnmapped(unmappedFields, aiMappings) {
        ...
        void 0 && console.log('★★★ Smart fallback: trying to map', fieldsToTry.length, 'AI-unmapped fields:', fieldsToTry);
        
        var smartMappings = performSmartMapping(fieldsToTry);
        
        if (Object.keys(smartMappings).length > 0) {
            void 0 && console.log('★★★ Smart fallback found', Object.keys(smartMappings).length, 'additional mappings:', smartMappings);
            applySmartMappings(smartMappings);
        } else {
            void 0 && console.log('★★★ Smart fallback: no additional mappings found');
        }
    }
    ...
    function autoMapCsvVariationAttributes(unmappedFields, aiMappings) {
        ...
        void 0 && console.log('★★★ Auto-mapping', attrColumns.length, 'CSV variation attribute columns:', attrColumns);
        ...
        void 0 && console.log('★★★ Auto-mapped', attrColumns.length, 'variation attributes');
    }
    ...
    function enableSkuAutoGenerate() {
        ...
        if ($skuRow.length === 0) {
            void 0 && console.log('★★★ SKU row not found');
            return;
        }
        ...
        if ($autoGenerateRadio.length > 0) {
            ...
            void 0 && console.log('★★★ SKU Auto-generate enabled with pattern:', $patternInput.val());
        } else {
            void 0 && console.log('★★★ SKU Auto-generate radio not found');
        }
    }
    ...
    function autoEnableManageStock() {
        ...
        if ($manageStockRow.length) {
            ...
            if ($yesRadio.length && !$yesRadio.prop('checked')) {
                ...
                void 0 && console.log('Auto-enabled Manage Stock because Stock Quantity is mapped');
            }
        }
    }
    ...
    function initializeProductTypeMode() {
        void 0 && console.log('★★★ initializeProductTypeMode() CALLED - 3-Card UI ★★★');
        
        // Handle click on mode cards
        $('.mode-card').on('click', function(e) {
            void 0 && console.log('★★★ Mode card clicked:', $(this).attr('id'));
        ...
        $('input[name="product_mode"]').on('change', function() {
            var mode = $(this).val();
            void 0 && console.log('★★★ Product mode changed to:', mode);
    ...
    function addDisplayAttributeRow(targetContainer) {
        targetContainer = targetContainer || 'display-attributes-list';
        void 0 && console.log('★★★ addDisplayAttributeRow() CALLED, target:', targetContainer);
        void 0 && console.log('window.allKnownFieldsOrder:', window.allKnownFieldsOrder);
        
        var index = $('.display-attribute-row').length;
        void 0 && console.log('Index:', index);
    ...
    function initializeAttributesAndVariations() {
        ...
        function populateAttributeSourceFields(index) {
            ...
            void 0 && console.log('Populating attribute fields for index:', index);
            void 0 && console.log('currentFileStructure:', window.currentFileStructure);
            ...
            if (structure && structure.length > 0) {
                void 0 && console.log('Structure has', structure.length, 'fields');
                structure.forEach(function(field) {
                ...
                void 0 && console.log('No file structure available yet');
            }
            
            void 0 && console.log('Setting options for selects:', $selectValues.length, $selectImage.length);
        ...
        function updateSkuPatternPreview() {
            ...
            var preview = pattern
                ...
                .replace(/\[/g, '')
                .replace(/\]/g, '');
            var _ip = (typeof bfpiImportData !== 'undefined' && bfpiImportData.i18n) ? bfpiImportData.i18n : {};
    ...
    function initializeStep3() {
        void 0 && console.log('Initializing Step 3: Import Progress');
    ...
    function loadFileStructure() {
        ...
        if (bfpiImportData.total_products && bfpiImportData.total_products > 0) {
            $('#total-products-count').text(bfpiImportData.total_products);
        ...
        var data = {
            ...
            file_path: bfpiImportData.file_path,
            file_type: bfpiImportData.file_type,
            product_wrapper: bfpiImportData.product_wrapper,
            total_products: bfpiImportData.total_products, // Pass from upload
            page: currentPage,
            per_page: 1, // Show 1 product per page for navigation
            nonce: bfpiImportData.nonce
        };

        void 0 && console.log('=== AJAX Request ===', data);
        void 0 && console.log('📤 Requesting page:', data.page, 'per_page:', data.per_page);
        ...
        $.ajax({
            ...
            success: function(response) {
                void 0 && console.log('=== AJAX Response ===', response);
                void 0 && console.log('📥 Received page:', response.data.current_page, 'Structure fields:', response.data.structure ? response.data.structure.length : 0);
                if (response.data.structure && response.data.structure.length > 0) {
                    void 0 && console.log('📥 First field:', response.data.structure[0].path, '=', response.data.structure[0].sample);
                }
                if (response.success) {
                    void 0 && console.log('Total products from backend:', response.data.total_products);
                    void 0 && console.log('Total pages from backend:', response.data.total_pages);
                    void 0 && console.log('Fields scanned from products:', response.data.fields_scanned_from);
                    ...
                    if (typeof bfpiImportData !== 'undefined' && bfpiImportData.saved_mappings && bfpiImportData.saved_mappings.attributes_variations) {
                        var csvConfig = bfpiImportData.saved_mappings.attributes_variations.csv_variation_config;
                        if (csvConfig) {
                            void 0 && console.log('★★★ Re-applying CSV variation config after dropdowns loaded:', csvConfig);
                    ...
                    if (typeof bfpiImportData !== 'undefined' && bfpiImportData.saved_mappings) {
                        void 0 && console.log('★★★ LOADING SAVED MAPPINGS (after dropdowns populated) ★★★');
                        loadSavedMappings(bfpiImportData.saved_mappings);
                    } else {
                        ...
                        var sourceFields = window.allKnownFieldsOrder || [];
                        if (sourceFields.length > 0) {
                            void 0 && console.log('★★★ Running Smart Auto-Mapping for', sourceFields.length, 'source fields');
            ...
            error: function(xhr, status, error) {
                ...
                // Fallback: initialize engines even if file structure loading failed
                if (!window._bfpiEnginesInitialized) {
                    window._bfpiEnginesInitialized = true;
                    void 0 && console.log('⚠️ File structure AJAX failed — initializing engines as fallback');
                    initPricingEngine();
                    initShippingClassEngine();
                    if (typeof bfpiImportData !== 'undefined' && bfpiImportData.saved_mappings) {
                        if (bfpiImportData.saved_mappings.pricing_engine) {
    ...
    function displayFileStructure(data) {
        var $container = $('#file-structure-browser');
        var html = '';
        var _i = (typeof bfpiImportData !== 'undefined' && bfpiImportData.i18n) ? bfpiImportData.i18n : {};
        ...
        if (data.sample_data && data.sample_data.length > 0) {
                ...
                if (Array.isArray(obj)) {
                    obj.forEach(function(item, idx) {
                        ...
                        if (typeof item === 'object' && item !== null) {
                            nestedHtml += '<div draggable="false" style="margin-left: ' + indent + 'px; margin-top: 5px; padding: 5px; background: #f8f9fa; border-radius: 3px; border-left: 2px solid #0073aa;">';
                            ...
                            for (var k in item) {
                                if (item.hasOwnProperty(k)) {
                                    ...
                                    if (typeof val === 'object' && val !== null) {
                                        nestedHtml += '<div draggable="false" style="margin-left: 10px;">';
                    ...
                    for (var k in obj) {
                        if (obj.hasOwnProperty(k)) {
                            ...
                            if (typeof val === 'object' && val !== null) {
                                nestedHtml += '<div draggable="false" style="margin-left: ' + indent + 'px; margin-top: 3px;">';
                                nestedHtml += '<span style="color: #666; font-size: 11px; font-weight: 500;">' + k + ':</span>';
                                nestedHtml += renderNestedObject(val, k, (level || 1) + 1, childPath);
                                nestedHtml += '</div>';
                            } else {
            ...
            function renderField(key, value) {
                ...
                if (isExpandable) {
                    fieldHtml += '<div id="' + currentExpandId + '" class="expandable-content" draggable="false" style="display: none; margin-top: 5px; max-height: 300px; overflow-y: auto;">';
        ...
        $container.find('#prev-product').off('click').on('click', function() {
            void 0 && console.log('🔵 Prev Product clicked - currentPage before:', currentPage);
            if (currentPage > 1) {
                currentPage--;
                void 0 && console.log('🔵 New currentPage:', currentPage, '- Loading structure...');
                loadFileStructure();
            }
        });
        
        $container.find('#next-product').off('click').on('click', function() {
            void 0 && console.log('🟢 Next Product clicked - currentPage before:', currentPage, 'totalPages:', totalPages);
            if (currentPage < totalPages) {
                currentPage++;
                void 0 && console.log('🟢 New currentPage:', currentPage, '- Loading structure...');
                loadFileStructure();
            } else {
                void 0 && console.log('🔴 Already at last page');
            }
        });
        ...
        $container.find('#goto-product-btn').off('click').on('click', function() {
            ...
            void 0 && console.log('🎯 Go to product clicked - target:', targetPage, 'totalPages:', totalPages);
            if (targetPage >= 1 && targetPage <= totalPages && targetPage !== currentPage) {
                currentPage = targetPage;
                void 0 && console.log('🎯 Jumping to product:', currentPage);
                loadFileStructure();
            }
        });
    ...
    function populateSourceFields(structure) {
        ...
        if (typeof bfpiImportData !== 'undefined' && bfpiImportData.existing_mappings && !window._mappingsRestored) {
            void 0 && console.log('Restoring existing mappings:', bfpiImportData.existing_mappings);
            $('.field-mapping-row').each(function() {
                var $row = $(this);
                var fieldKey = $row.data('field');
                var mapping = bfpiImportData.existing_mappings[fieldKey];
                ...
                if (mapping && mapping.source) {
                    ...
                    if ($select.length) {
                        ...
                        void 0 && console.log('Restored (select)', fieldKey, '→', mapping.source);
                    }
                    ...
                    if ($textarea.length) {
                        ...
                        void 0 && console.log('Restored (textarea)', fieldKey, '→', mapping.source);
                    }
                }
            });
        ...
        void 0 && console.log('📊 Total unique fields discovered: ' + fieldCount);
    }
    ...
    function addFilterRule(e) {
        ...
        void 0 && console.log('Container found:', $container.length);
        ...
        void 0 && console.log('Template found:', template ? 'yes' : 'no');
        
        var index = Date.now(); // Simple unique index

        var html = template.replace(/\{index\}/g, index);
        $container.append(html);
        void 0 && console.log('Filter row added');
    ...
    function loadSavedMappings(savedMappings) {
        void 0 && console.log('★★★ loadSavedMappings called with:', savedMappings);
        
        if (!savedMappings || typeof savedMappings !== 'object') {
            void 0 && console.log('No saved mappings to load');
            return;
        }
        ...
        $.each(savedMappings, function(fieldName, fieldConfig) {
            ...
            void 0 && console.log('Loading mapping for field:', fieldName, fieldConfig);
            ...
            if (fieldName === 'shipping_class_formula') {
                if (fieldConfig.formula) {
                    ...
                    void 0 && console.log('Loaded shipping class formula');
                }
                return;
            }
            ...
            if ($booleanRadio.length > 0 && fieldConfig.boolean_mode) {
                ...
                void 0 && console.log('Loaded boolean field:', fieldName, fieldConfig.boolean_mode);
            ...
            if ($selectRadio.length > 0 && fieldConfig.select_mode) {
                ...
                void 0 && console.log('Loaded select field:', fieldName, fieldConfig.select_mode, fieldConfig.fixed_value);
                return; // Done with this field
            }
            ...
            void 0 && console.log('★ loadSavedMappings for', fieldName, '| select:', $sourceSelect.length, '| textarea:', $sourceTextarea.length, '| source:', fieldConfig.source);
            ...
            if ($sourceSelect.length > 0 || $sourceTextarea.length > 0) {
                ...
                if (fieldConfig.source) {
                    ...
                    void 0 && console.log('Set source for', fieldName, 'to', fieldConfig.source);
                }
                ...
                if (fieldConfig.sku_mode) {
                    ...
                    if ($skuModeRadio.length > 0) {
                        ...
                        void 0 && console.log('Loaded SKU mode (in source block):', fieldName, 'mode:', fieldConfig.sku_mode, 'pattern:', fieldConfig.sku_pattern);
                    }
                }
            }
        });
        ...
        if (savedMappings.attributes_variations) {
            void 0 && console.log('★★★ Loading Attributes & Variations:', savedMappings.attributes_variations);
        ...
        void 0 && console.log('★★★ Finished loading saved mappings');
    }
    ...
    function loadSavedAttributesVariations(config) {
        ...
        void 0 && console.log('★★★ loadSavedAttributesVariations called with:', config);
        ...
        void 0 && console.log('★★★ Restored product_mode:', productMode);
        ...
        if (attrInputMode === 'key_value') {
            ...
            void 0 && console.log('★★★ Restored attr_input_mode: key_value');
            
            // Load attribute pairs
            if (config.attribute_pairs && config.attribute_pairs.length > 0) {
                void 0 && console.log('★★★ Loading attribute_pairs:', config.attribute_pairs.length);
                ...
                config.attribute_pairs.forEach(function(pair) {
                    if (pair.name_field && pair.value_field) {
                        ...
                        void 0 && console.log('★★★ Restored attribute pair:', pair.name_field, '→', pair.value_field);
                    }
                });
            }
        } else {
            ...
            if (config.display_attributes && config.display_attributes.length > 0) {
                void 0 && console.log('★★★ Loading display_attributes:', config.display_attributes.length);
                ...
                config.display_attributes.forEach(function(dispAttr) {
                    if (dispAttr.name) {
                        ...
                        void 0 && console.log('★★★ Restored display attribute:', dispAttr.name, '→', dispAttr.source);
                    }
                });
            }
        }
        ...
        if (config.variation_path) {
            ...
            void 0 && console.log('★★★ Restored variation_path:', config.variation_path);
        }
        ...
        if (config.attribute_detection_mode) {
            ...
            void 0 && console.log('★★★ Restored attribute_detection_mode:', config.attribute_detection_mode);
        ...
        if (config.variation_attributes && config.variation_attributes.length > 0) {
            void 0 && console.log('★★★ Loading variation_attributes:', config.variation_attributes.length);
            ...
            config.variation_attributes.forEach(function(varAttr) {
                if (varAttr.name) {
                    ...
                    void 0 && console.log('★★★ Restored variation attribute:', varAttr.name, '→', varAttr.source, 'array_index:', varAttr.array_index);
                }
            });
        }
        ...
        if (config.variation_fields) {
            Object.keys(config.variation_fields).forEach(function(fieldKey) {
                ...
                void 0 && console.log('★★★ Restored var_field[' + fieldKey + ']:', value);
            });
        }
        ...
        if (config.variation_meta && config.variation_meta.length > 0) {
            config.variation_meta.forEach(function(metaItem) {
                if (metaItem.key) {
                    ...
                    void 0 && console.log('★★★ Restored variation meta:', metaItem.key, '→', metaItem.source);
                }
            });
        }
        ...
        if (config.csv_variation_config) {
            ...
            void 0 && console.log('★★★ Loading csv_variation_config:', csvConfig);
            
            // Restore parent SKU column
            if (csvConfig.parent_sku_column) {
                $('#csv-parent-sku-column').val(csvConfig.parent_sku_column);
                void 0 && console.log('★★★ Restored CSV parent_sku_column:', csvConfig.parent_sku_column);
            }
            
            // Restore type column
            if (csvConfig.type_column) {
                $('#csv-type-column').val(csvConfig.type_column);
                void 0 && console.log('★★★ Restored CSV type_column:', csvConfig.type_column);
            }
            ...
            if (csvConfig.attributes && csvConfig.attributes.length > 0) {
                ...
                csvConfig.attributes.forEach(function(attr) {
                    if (attr.name) {
                        ...
                        void 0 && console.log('★★★ Restored CSV var attribute:', attr.name, '→', attr.source);
                    }
                });
            }
            ...
            if (csvConfig.fields) {
                Object.keys(csvConfig.fields).forEach(function(fieldKey) {
                    ...
                    void 0 && console.log('★★★ Restored csv_var_field[' + fieldKey + ']:', value);
                });
            }
        }
        
        void 0 && console.log('★★★ Attributes & Variations loaded');
    }
    ...
    function loadSavedCustomFields(savedFields) {
        void 0 && console.log('Loading saved custom fields:', savedFields);
        ...
        void 0 && console.log('Loaded ' + savedFields.length + ' custom fields');
    }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    void 0 && console.log('=== AI TEST FULL RESULT ===');
                    void 0 && console.log('Test Value:', testValue);
                    void 0 && console.log('Result:', response.data.result);
                    void 0 && console.log('===========================');
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    void 0 && console.log('=== PHP Formula Test ===');
                    void 0 && console.log('Input:', testValue);
                    void 0 && console.log('Formula:', formula);
                    void 0 && console.log('Result:', response.data.result);
                    void 0 && console.log('=======================');
    ...
    function startImportAfterSave(mappingData) {
        ...
        // Add import data from bfpiImportData if available
        if (typeof bfpiImportData !== 'undefined') {
            ...
            if (importIdFromUrl) {
                ...
                void 0 && console.log('★★★ Adding import_id to Start Import:', importIdFromUrl);
            }
            
            if (bfpiImportData.import_name) {
                formData.append('import_name', bfpiImportData.import_name);
            }
            if (bfpiImportData.schedule_type) {
                formData.append('schedule_type', bfpiImportData.schedule_type);
            }
            if (bfpiImportData.file_path) {
                formData.append('file_path', bfpiImportData.file_path);
            }
            if (bfpiImportData.file_type) {
                formData.append('file_type', bfpiImportData.file_type);
            }
            if (bfpiImportData.product_wrapper) {
                formData.append('product_wrapper', bfpiImportData.product_wrapper);
            }
        }
        
        // CRITICAL: Add update_existing and skip_unchanged from mappingData (current checkbox state)
        // NOT from bfpiImportData (old database value)
    ...
    function saveMapping() {
        void 0 && console.log('★★★ SAVE MAPPING CLICKED ★★★');
        ...
        var mappingData = collectMappingData();
        void 0 && console.log('Collected mapping data:', mappingData);
        
        // Get import ID
        var importId = getImportIdFromUrl();
        void 0 && console.log('Import ID from URL:', importId);
        ...
        void 0 && console.log('Sending AJAX request...');
    ...
    function collectMappingData() {
        ...
        $('.field-mapping-row:not(.custom-field-row)').each(function() {
            ...
            if ($textareaEl.length) {
                void 0 && console.log('📝 Field:', fieldKey, '| Textarea found:', $textareaEl.length, '| Value:', textareaVal);
            }
            ...
            if ($skuModeRadio.length) {
                ...
                void 0 && console.log('★ SKU mode collected:', data.field_mapping[fieldKey].sku_mode, 'pattern:', data.field_mapping[fieldKey].sku_pattern);
            }
        ...
        void 0 && console.log('COLLECTED MAPPING DATA:', data);
        ...
        if (shippingFormula && shippingFormula.trim()) {
            ...
            void 0 && console.log('★ Collected shipping_class_formula:', shippingFormula.substring(0, 50) + '...');
        }
        ...
        void 0 && console.log('COLLECTED FILTERS:', data.import_filters, 'Logic:', data.filter_logic, 'Draft non-matching:', data.draft_non_matching);
        void 0 && console.log('★ COLLECTED SETTINGS: update_existing=' + data.update_existing + ', skip_unchanged=' + data.skip_unchanged + ', batch_size=' + data.batch_size + ', schedule_type=' + data.schedule_type + ', schedule_method=' + data.schedule_method);
        ...
        $('.custom-field-row').each(function() {
            ...
            if (name && sourceField) {
                ...
                void 0 && console.log('Collected custom field:', fieldData);
            }
        });
        ...
        if (productMode === 'attributes') {
            ...
            if (attrInputMode === 'key_value') {
                ...
                void 0 && console.log('★★★ Key-Value Pairs:', attributesData.attribute_pairs);
            } else {
        ...
        if (productMode === 'variable') {
            ...
            var fileType = (typeof bfpiImportData !== 'undefined' && bfpiImportData.file_type) || 'xml';
            ...
            if (fileType === 'csv') {
                ...
                void 0 && console.log('★★★ CSV VARIATION CONFIG:', attributesData.csv_variation_config);
            } else {
                ...
                if (attrDetectionMode === 'auto') {
                    ...
                    void 0 && console.log('★★★ AUTO-DETECT MODE - path:', attributesData.auto_attributes_path);
                } else {
        ...
        void 0 && console.log('★★★ PRODUCT MODE:', productMode);
        void 0 && console.log('★★★ COLLECTED ATTRIBUTES & VARIATIONS:', attributesData);
        ...
        if (typeof window.getPricingEngineConfig === 'function') {
            ...
            void 0 && console.log('★★★ COLLECTED PRICING ENGINE:', pricingEngineConfig);
        }
        ...
        if (typeof window.getShippingClassEngineConfig === 'function') {
            ...
            void 0 && console.log('★★★ COLLECTED SHIPPING CLASS ENGINE:', shippingClassEngineConfig);
        }
        ...
        if ($catBlock.length) {
            ...
            void 0 && console.log('★★★ COLLECTED CATEGORIES:', catData);
        }
        ...
        if ($tagBlock.length) {
            ...
            void 0 && console.log('★★★ COLLECTED TAGS:', tagData);
        }
        ...
        if ($brandBlock.length) {
            ...
            void 0 && console.log('★★★ COLLECTED BRAND:', brandData);
        }
    ...
    function triggerNextBatch(importId, offset) {
        ...
        var batchSize = (typeof bfpiImportData !== 'undefined' && bfpiImportData.batch_size) 
            ? parseInt(bfpiImportData.batch_size) 
            : 50;
        ...
        $.ajax({
            ...
            success: function(response) {
                void 0 && console.log('Batch processed:', response);
            },
            error: function(xhr, status, error) {
    ...
    function populateFilterDropdowns() {
        if (!window.currentFileStructure || !window.currentFileStructure.length) {
            void 0 && console.log('No file structure available for filter dropdowns');
            return;
        }
        
        void 0 && console.log('Populating filter dropdowns with fields:', window.currentFileStructure);
    ...
    function loadRecipesList() {
        void 0 && console.log('loadRecipesList() called');
        ...
        $.ajax({
            ...
            success: function(response) {
                void 0 && console.log('Recipes response:', response);
                if (response.success && response.data.recipes) {
                    ...
                    void 0 && console.log('Loaded ' + response.data.recipes.length + ' recipes');
                } else {
    ...
    $(document).on('click', '#save-recipe-btn', function() {
        ...
        var mappingData = collectMappingData();
        void 0 && console.log('Saving recipe with mapping data:', mappingData);
    ...
    $(document).on('click', '#load-recipe-btn', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                void 0 && console.log('Load recipe response:', response);
                if (response.success && response.data.recipe) {
                    void 0 && console.log('Applying mapping data:', response.data.recipe.mapping_data);
    ...
    $(document).on('click', '#auto-detect-mapping-btn', function() {
        ...
        void 0 && console.log('Auto-detect: Source fields:', sourceFields);
        ...
        $.ajax({
            ...
            success: function(response) {
                void 0 && console.log('Auto-detect response:', response);
                    ...
                    $.each(response.data.suggestions, function(wooField, suggestion) {
                        ...
                        if ($row.length) {
                            ...
                            if ($booleanMapRadio.length) {
                                ...
                                appliedCount++;
                                void 0 && console.log('Applied boolean mapping:', wooField, '->', suggestion.source_field);
                            }
                            // Check if this is a select field (has Fixed/Map radio buttons)
                                ...
                                appliedCount++;
                                void 0 && console.log('Applied select mapping:', wooField, '->', suggestion.source_field);
                            }
                            // Regular text field
                            else {
                                var $select = $row.find('.field-source-select');
                                if ($select.length && $select.find('option[value="' + suggestion.source_field + '"]').length) {
                                    ...
                                    appliedCount++;
                                    void 0 && console.log('Applied text mapping:', wooField, '->', suggestion.source_field);
                                }
                            }
    ...
    function applyMappingData(data) {
        void 0 && console.log('applyMappingData called with:', data);
        if (!data) {
        ...
        if (data.field_mapping) {
            void 0 && console.log('Applying field_mapping:', Object.keys(data.field_mapping));
            $.each(data.field_mapping, function(fieldKey, fieldData) {
                var $row = $('.field-mapping-row[data-field="' + fieldKey + '"]');
                if (!$row.length) {
                    void 0 && console.log('Row not found for:', fieldKey);
                    return;
                }
                void 0 && console.log('Applying to field:', fieldKey, fieldData);
        ...
        if (data.field_mapping && data.field_mapping.attributes_variations) {
            ...
            if (attrData.variation_fields) {
                void 0 && console.log('Applying variation_fields:', attrData.variation_fields);
                $.each(attrData.variation_fields, function(fieldKey, value) {
                    if (value) {
                        $('textarea[name="var_field[' + fieldKey + ']"]').val(value);
                        void 0 && console.log('Set var_field[' + fieldKey + ']:', value);
                    }
                });
            }
        ...
        void 0 && console.log('Mapping data applied from recipe');
    }
    ...
    function initDeleteProductsWithProgress() {
        ...
        function getI18n(key, defaultValue) {
            if (typeof bfpiImportData !== 'undefined' && bfpiImportData.i18n && bfpiImportData.i18n[key]) {
                return bfpiImportData.i18n[key];
            }
            return defaultValue;
        }
        ...
        // Get nonce safely
        function getNonce() {
            if (typeof bfpiImportData !== 'undefined' && bfpiImportData.nonce) {
                return bfpiImportData.nonce;
            }
            return '';
        }
        ...
        if ($('#delete-products-modal').length === 0) {
            var modalHtml = '<div id="delete-products-modal" class="bfpi-modal" style="display:none;">' +
                '<div class="bfpi-modal-content delete-progress-modal">' +
        ...
        $(document).on('click', '.delete-products-ajax', function(e) {
            ...
            void 0 && console.log('Delete Products AJAX - Import ID:', importId, 'Nonce:', nonceToUse);
            ...
            $.ajax({
                ...
                success: function(response) {
                    void 0 && console.log('Get products count response:', response);
                    if (response.success) {
    ...
    function initTextareaMappingUI() {
        void 0 && console.log('🎯 Initializing Textarea Mapping UI...');
        ...
        void 0 && console.log('✅ Textarea Mapping UI initialized');
    }
    ...
    window.collectNestedFieldPaths = function() {
        ...
        void 0 && console.log('📊 Collected ' + window.allKnownFieldsOrder.length + ' total fields for autocomplete');
    };
    ...
    function initDragSources() {
        ...
        $(document).on('mousedown', '.product-data-preview [data-field-path]', function(e) {
            e.stopPropagation();
            // Ensure only this element is draggable, disable on ancestors
            $(this).attr('draggable', 'true');
            $(this).parents('[draggable]').attr('draggable', 'false');
        });
    ...
    function initDropTargets() {
        ...
        $(document).on('drop', '.field-mapping-textarea', function(e) {
            ...
            void 0 && console.log('📥 Dropped:', droppedText, 'into', $textarea.attr('name'));
        });
    }
        ...
        if ($('.bfpi-step-2').length) {
            ...
            var checkInterval = setInterval(function() {
                if (window.allKnownFieldsOrder && window.allKnownFieldsOrder.length > 0) {
                    ...
                    if (typeof bfpiImportData !== 'undefined' && bfpiImportData.saved_mappings && bfpiImportData.saved_mappings.pricing_engine) {
                        void 0 && console.log('★★★ Restoring Pricing Engine config after init:', bfpiImportData.saved_mappings.pricing_engine);
                        window.restorePricingEngineConfig(bfpiImportData.saved_mappings.pricing_engine);
                    }
                    
                    // Restore Shipping Class Engine config after initialization
                    if (typeof bfpiImportData !== 'undefined' && bfpiImportData.saved_mappings && bfpiImportData.saved_mappings.shipping_class_engine) {
                        void 0 && console.log('★★★ Restoring Shipping Class Engine config after init:', bfpiImportData.saved_mappings.shipping_class_engine);
                        window.restoreShippingClassEngineConfig(bfpiImportData.saved_mappings.shipping_class_engine);
                    }
                }
            }, 500);
            
            // Timeout after 30 seconds — initialize engines even without file structure
            setTimeout(function() {
                clearInterval(checkInterval);
                // If engines haven't been initialized yet (allKnownFieldsOrder was empty), do it now
                if (!window._bfpiEnginesInitialized) {
                    window._bfpiEnginesInitialized = true;
                    void 0 && console.log('⏰ Timeout: initializing engines without file structure');
                    initPricingEngine();
                    initShippingClassEngine();
                    if (typeof bfpiImportData !== 'undefined' && bfpiImportData.saved_mappings) {
                        if (bfpiImportData.saved_mappings.pricing_engine) {
    ...
    function initPricingEngine() {
        void 0 && console.log('★★★ initPricingEngine() CALLED ★★★');
    ...
    window.getPricingEngineConfig = function() {
        ...
        void 0 && console.log('★★★ PRICING ENGINE CONFIG:', config);
        return config;
    };
    ...
    window.restorePricingEngineConfig = function(config) {
        ...
        void 0 && console.log('★★★ Restoring Pricing Engine config:', config);
    ...
    function initShippingClassEngine() {
        void 0 && console.log('★★★ initShippingClassEngine() CALLED ★★★');
        ...
        // Enable drag-and-drop reordering of shipping rules
        $('#shipping-class-rules-list').sortable({
            handle: '.rule-drag-handle',
            items: '.shipping-rule-conditional',
            axis: 'y',
            opacity: 0.7,
            placeholder: 'shipping-rule-sortable-placeholder',
            tolerance: 'pointer',
            update: function() {
                updateShippingRuleNumbers();
            }
    ...
    window.getShippingClassEngineConfig = function() {
        ...
        void 0 && console.log('★★★ SHIPPING CLASS ENGINE CONFIG:', config);
        return config;
    };
    ...
    window.restoreShippingClassEngineConfig = function(config) {
        ...
        void 0 && console.log('★★★ Restoring Shipping Class Engine config:', config);
    ...
    $(document).ready(function() {
        ...
        $dropdown.on('click', '.bootflow-lang-option', function(e) {
            ...
            // AJAX save
            var ajaxUrl = (typeof bfpi_ajax !== 'undefined') ? bfpi_ajax.ajax_url : 
                          (typeof bfpi_ajax !== 'undefined') ? bfpi_ajax.ajax_url :
                          (typeof bfpiImportData !== 'undefined') ? bfpiImportData.ajax_url :
                          (typeof ajaxurl !== 'undefined') ? ajaxurl : '/wp-admin/admin-ajax.php';
            
            $.post(ajaxUrl, {
                action: 'bfpi_switch_language',
                locale: locale,
                nonce: nonce
            }, function(response) {
                if (response.success) {
    ...
    function initializeTaxonomyMapping() {
        void 0 && console.log('BPI: initializeTaxonomyMapping() called');
        ...
        function bpiBuildCategoryOptions(selectedId) {
            var cats = (typeof bfpiCategories !== 'undefined') ? bfpiCategories : [];
            var html = '<option value="">' + ((typeof bfpiI18n !== 'undefined' && bfpiI18n.select_category) ? bfpiI18n.select_category : '— Select Category —') + '</option>';
            html += '<option value="__new__">' + ((typeof bfpiI18n !== 'undefined' && bfpiI18n.create_new) ? bfpiI18n.create_new : '+ Create New') + '</option>';
        ...
        function bpiBuildTagOptions(selectedId) {
            var tags = (typeof bfpiTags !== 'undefined') ? bfpiTags : [];
            var html = '<option value="">' + ((typeof bfpiI18n !== 'undefined' && bfpiI18n.select_tag) ? bfpiI18n.select_tag : '— Select Tag —') + '</option>';
            html += '<option value="__new__">' + ((typeof bfpiI18n !== 'undefined' && bfpiI18n.create_new) ? bfpiI18n.create_new : '+ Create New') + '</option>';
        ...
        function bpiBuildBrandOptions(selectedId) {
            var brands = (typeof bfpiBrands !== 'undefined') ? bfpiBrands : [];
            var html = '<option value="">' + ((typeof bfpiI18n !== 'undefined' && bfpiI18n.select_brand) ? bfpiI18n.select_brand : '— Select Brand —') + '</option>';
            html += '<option value="__new__">' + ((typeof bfpiI18n !== 'undefined' && bfpiI18n.create_new) ? bfpiI18n.create_new : '+ Create New') + '</option>';
        ...
        $('#btn-add-cat-mapping-row').on('click', function(e) {
            e.preventDefault();
            void 0 && console.log('BPI: Add cat mapping row clicked');
        ...
        $('#btn-add-tag-mapping-row').on('click', function(e) {
            e.preventDefault();
            void 0 && console.log('BPI: Add tag mapping row clicked');
        ...
        $('#btn-add-brand-mapping-row').on('click', function(e) {
            e.preventDefault();
            void 0 && console.log('BPI: Add brand mapping row clicked');
        ...
        void 0 && console.log('BPI: Taxonomy mapping handlers registered OK');
    }
